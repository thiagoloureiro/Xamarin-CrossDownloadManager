                ...
                foo.File.PropertyChanged += (sender, e) =>
                {
                    ...
                    switch (e.PropertyName)
                    {
                        ...
                        case nameof(IDownloadFile.TotalBytesExpected):
                            FindViewById<TextView>(Resource.Id.value_totalbytesexpected).Text = (downloadFile.TotalBytesExpected / 1000).ToString();
                            break;

                        case nameof(IDownloadFile.TotalBytesWritten):
                            FindViewById<TextView>(Resource.Id.value_totalbyteswritten).Text = (downloadFile.TotalBytesWritten / 1000).ToString();
                            break;
                    }
